...
#ifndef IBUNDLECONTEXT_H_
#define IBUNDLECONTEXT_H_
 ...
 *
 * <p>
 * A <code>IBundleContext</code> object will be created and provided to the
 * bundle associated with this context when it is loaded using the
 * {@link BundleActivator::Load} method. The same <code>IBundleContext</code>
 * object will be passed to the bundle associated with this context when it is
 * unloaded using the {@link BundleActivator::Unload} method. A
 * <code>IBundleContext</code> object is generally for the private use of its
 * associated bundle and is not meant to be shared with other modules in the
 * bundle environment.
 *
 * <p>
 * The <code>IBundle</code> object associated with a <code>IBundleContext</code>
 * object is called the <em>context bundle</em>.
 *
 * <p>
 * The <code>IBundleContext</code> object is only valid during the execution of
 * its context bundle; that is, during the period when the context bundle
 * is loaded. If the <code>IBundleContext</code>
 * object is used subsequently, a <code>std::logic_error</code> is
 * thrown. The <code>IBundleContext</code> object is never reused after
 * its context bundle is unloaded.
 *
 * <p>
 * The framework is the only entity that can create <code>IBundleContext</code>
 * objects.
 *
...
public:
   ...
   * <code>IBundleContext</code>. This bundle is called the context bundle.
   *
   * @return The <code>IBundle</code> object associated with this
   *         <code>IBundleContext</code>.
   * @throws std::logic_error If this IBundleContext is no
   *         longer valid.
   */
  virtual IBundle* GetBundle() const = 0;
   ...
   */
  virtual IBundle* GetBundle(long id) const = 0;
   ...
   */
  virtual std::vector<IBundle*> void GetBundles() const = 0;
   ...
   *         </ul>
   * @throws std::logic_error If this IBundleContext is no longer valid.
   * @see ServiceRegistration
   * @see ServiceFactory
   */
  ServiceRegistration RegisterService(const std::vector<std::string>& clazzes,
                                      const std::vector<void*>& servicePointers,
   ...
   */
  template<class S>
  ServiceRegistration RegisterService(S* service, const ServiceProperties& properties = ServiceProperties())
  {
   ...
   * @throws std::logic_error If this IBundleContext is no longer valid.
   */
  std::vector<ServiceReference> GetServiceReferences(const std::string& clazz,
   ...
   */
  template<class S>
  std::vector<ServiceReference> GetServiceReferences(const std::string& filter = std::string())
  {
    const char* clazz = us_service_interface_iid<S*>();
   ...
   * {@link IBundleContext::GetServiceReferences(const std::string&, const std::string&)} with an
   ...
   * @see ServiceFactory
   */
  void* GetService(const ServiceReference& reference);
  ...
  {
    return reinterpret_cast<S*>(GetService(reference));
  }
...
private:
  ...
  friend class IBundle;
  friend class BundlePrivate;

  IBundleContext(BundlePrivate* bundle);

  // purposely not implemented
  IBundleContext(const IBundleContext&);
  IBundleContext& operator=(const IBundleContext&);

  IBundleContextPrivate * const d;
};
